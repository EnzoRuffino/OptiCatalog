...
const page = ref<CollectionDetail | null>(null);
...
useSeo(
  () => `${page.value?.label ?? 'Collection'} | SEO catégorie`,
  () =>
    `Optimise la page liste (PLP) pour le référencement : titre, texte d’intro et mots-clés.`,
);
...
async function load() {
  ...
  try {
    ...
    error.value = 'Page collection introuvable.';
  } finally {
    loading.value = false;
  }
}
...
async function optimize() {
  ...
  try {
    const { data } = await api.post<JobResponse>(
      `/projects/${projectId}/collections/${collectionId}/optimize`,
    );
...
<template>
  <div class="space-y-10 md:space-y-12">
    ...
    <p v-if="loading" class="py-8 text-wood-500">Chargement…</p>

    <template v-else-if="page">
      <header class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div class="max-w-3xl space-y-2">
          <p class="oc-label">Catégorie / collection</p>
      ...
      <div class="grid gap-8 lg:grid-cols-2 lg:gap-10">
        <section class="oc-card-solid p-8 md:p-10">
          <h2 class="oc-label text-clay-600">Contexte fourni</h2>
          <p class="mt-4 font-display text-xl font-medium text-wood-900">{{ page.label }}</p>
          <p
            v-if="page.originalDescription.trim()"
            class="mt-4 whitespace-pre-wrap leading-relaxed text-wood-700"
          >
            {{ page.originalDescription }}
          </p>
          <p v-else class="mt-4 text-wood-500">Aucun texte de contexte — l’IA s’appuie sur le nom de la collection.</p>
        </section>

        <section
          class="rounded-3xl border border-moss-200/90 bg-moss-50/70 p-8 shadow-soft md:p-10"
        >
          <h2 class="oc-label text-moss-600">Après · IA (PLP)</h2>
          <template v-if="page.generation">
            <p class="mt-4 font-display text-xl font-medium text-wood-900">
              {{ page.generation.optimizedTitle }}
            </p>
            <p class="mt-4 whitespace-pre-wrap leading-relaxed text-wood-800">
              {{ page.generation.optimizedDescription }}
            </p>
            <p class="mt-6 text-sm text-wood-700">
              <span class="font-medium text-wood-800">Mots-clés :</span>
              {{ page.generation.seoKeywords }}
            </p>
            <p class="mt-3 inline-flex items-center gap-2 rounded-full bg-white/80 px-4 py-2 text-sm font-semibold text-moss-600 shadow-sm">
              Score SEO {{ page.generation.seoScore }} / 100
            </p>
          </template>
          <p v-else class="mt-6 leading-relaxed text-wood-600">
            Pas encore de version optimisée — lance l’IA pour générer titre, texte d’intro liste et signaux SEO.
          </p>
        </section>
      </div>
    </template>
